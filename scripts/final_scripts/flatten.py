...
class TransE(nn.Module):
    ...
    def forward(self,subjects,objects,relations):
        sub = self.linear(torch.flatten(self.ent_embedding(subjects).squeeze(1),start_dim=1))
        obj = self.linear(torch.flatten(self.ent_embedding(objects).squeeze(1),start_dim=1))
        rel = self.linear(torch.flatten(self.rel_embedding(relations).squeeze(1),start_dim=1))
...
if __name__ == "__main__":
    ...
    #torch.save(model,'./flatten.model')
    #exit(0)
    model = torch.load('./flatten.model')
    subjects, objects, relations = torch.chunk(torch.LongTensor(data.test).cuda(0), 3, dim=1)
    sub = model.linear(torch.flatten(model.ent_embedding(subjects).squeeze(1),start_dim=1))
    obj = model.linear(torch.flatten(model.ent_embedding(objects).squeeze(1),start_dim=1))
    rel = model.linear(torch.flatten(model.rel_embedding(relations).squeeze(1),start_dim=1))
    print(torch.sum(torch.sum((sub+rel-obj)**2,-1)))
    with open('test_flatten.pkl','wb') as f:
