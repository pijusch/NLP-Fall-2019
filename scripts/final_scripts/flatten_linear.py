...
class TransE(nn.Module):
    def __init__(self, data, DIM_EMB=100, DIM_LSTM=100):
        ...
        self.linear_s = nn.Linear(3*DIM_EMB,DIM_EMB)
        self.linear_p= nn.Linear(3*DIM_EMB,DIM_EMB)
        self.linear_o = nn.Linear(3*DIM_EMB,DIM_EMB)
    ...
    def forward(self,subjects,objects,relations):
        sub = self.linear_s(torch.flatten(self.ent_embedding(subjects).squeeze(1),start_dim=1))
        obj = self.linear_s(torch.flatten(self.ent_embedding(objects).squeeze(1),start_dim=1))
        rel = self.linear_p(torch.flatten(self.rel_embedding(relations).squeeze(1),start_dim=1))
...
if __name__ == "__main__":
    ...
    #torch.save(model,'./flatten2.model')
    #exit(0)
    model = torch.load('./flatten2.model')
    subjects, objects, relations = torch.chunk(torch.LongTensor(data.test).cuda(0), 3, dim=1)
    sub = model.linear_s(torch.flatten(model.ent_embedding(subjects).squeeze(1),start_dim=1))
    obj = model.linear_s(torch.flatten(model.ent_embedding(objects).squeeze(1),start_dim=1))
    rel = model.linear_p(torch.flatten(model.rel_embedding(relations).squeeze(1),start_dim=1))
    print(torch.sum(torch.sum((sub+rel-obj)**2,-1)))
    with open('test_flatten2.pkl','wb') as f:
