...
def hitsatk_transe(spo, k):
    ...
    #e = torch.LongTensor(np.arange(model.num_ent)).repeat(valid_batch).view(-1,1)
...
def run_transe_validation(data):
    ...
    return [(float(sum(hits1)) / len(hits1)),(float(sum(hits1)) / len(hits10)),(float(sum(hits100)) / len(hits100))]
